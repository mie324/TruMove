...
class SportIntroController: UIViewController, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    //MARK: START SPORT BUTTON
    @objc func handleStart(){
        //save to firebase
        guard let uid = Auth.auth().currentUser?.uid else {return}
        if statusLabel.text != "Connected" {
            startsportButton.isEnabled = false
            let alert = UIAlertController(title: "Warning", message: "Please checking the connect status", preferredStyle: .alert)
        ...
        Firestore.firestore().collection(uid).addDocument(data:[
                 ...
                 "y_value": self.yArray,
                 "z_value": self.zArray,
                 "timestamp": Date().timeIntervalSince1970]){ err in
            if let err = err {
